...
contract GasTest is Test {
    ...
    function printCode(address target) internal view {
        uint256 size;
        assembly {
            size := extcodesize(target)
        }
        bytes memory codeBytes = new bytes(size);

        assembly {
            extcodecopy(target, add(codeBytes, 0x20), 0, size)
        }
        console.log("bytes: ");
    ...
    function test_admins() public view {
        printCode(address(gas));

        for (uint8 i = 0; i < admins.length; ++i) {
            console.log("admin[%d]: %x", i, admins[i]);
    ...
    function testTransfer(uint256 _amount, address _recipient) public {
        ...
        //
        uint256 ownerBal = gas.balanceOf(owner); // 		totalSupply + amount	totalSupply + amount
        uint256 balBefore = gas.balanceOf(_recipient); // 	call2  					call2
        gas.transfer(_recipient, _amount, "name");
        uint256 balAfter = gas.balanceOf(_recipient); // 	call3==call2 + 0		call3 == call2 + amount

        if (_recipient == owner) {
            assertEq(balAfter, balBefore);
            assertEq(gas.balanceOf(owner), ownerBal); //	totalSupply - 0
        } else {
            assertEq(balAfter, balBefore + _amount);
            assertEq(gas.balanceOf(owner), ownerBal - _amount); //					totalSupply - _amount
        }
    }
}
